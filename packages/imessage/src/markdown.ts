...
export interface TextFormatInput {
  length: number;
  start: number;
  type: "bold" | "italic" | "strikethrough" | "underline" | "effect";
  effect_name?: string;
}
...
// Same row layout as the plain-text renderer ("h1 | h2" lines). Emphasis
// ranges flow through cell text.
...
const renderBlockToken = (token: MarkedToken): StyledSpan[] => {
  switch (token.type) {
    // iMessage formatting has no heading sizes; bold is the conventional stand-in.
    case "heading":
      return withStyle(renderInlineTokens(token.tokens), "bold");
    case "paragraph":
